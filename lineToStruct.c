...
static void assign_operand_option(syntax_tree * st, int index, char operand_opt) {
    /*assigns an operand type to a specific index in the syntax tree's array of operand options.*/
...
static void check_directive(syntax_tree *st, char *operands_string, struct directive_opt  * optD) {
    ...
    if(optD->key <= directive_entry) {
        ...
        if(!split || !(split2=strrchr(split+1,'"'))) {
            sprintf(st->syntax_error, "directive: '%s' has no %s '\"': '%s'.",optD->directive_name, split?"closing":"opening",operands_string);
            return;
        }
        split++;
        *split2 = '\0';
